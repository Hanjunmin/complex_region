...
rule gene_simapa: ##yes
    input:
        W="{base_dir}/02_refSD/sam_cor/Wpath/{line}.W",  # WCOM=rules.gene_Wpath.output,
    ...
    shell:
        ...
        python {script_dir}/ref_SD/sam_sd/scripts/iterpansd.py  --W {input.W} --S {wildcards.line} --i "{base_dir}03_nonref/01_simpath/{wildcards.line}alldf.e.txt" --o {output.output_bed} --p {params.dbpath} > {output.time_output} 2> {output.time_log} ##这里面有个bedtools路径需要改 iterpansd.py 也可以改为两步骤 iterpansd1.py iterpansd2.py  
        '''

###################### SD calculation
rule process_line1: ##yes
    input:
        samwin="{base_dir}/02_refSD/sam_cor/Wpath/{line}.W",
        sammiin="{base_dir}/02_refSD/sam_cor/2_ALIGN/other/{line}.other",
...
rule prallel_kmercal: ##yes
    ...
    shell:
        ...
        python {script_dir}/nonref_SD/scripts/para_kmer_wri.py --script {script_dir}/nonref_SD/scripts/filterpair.r
        """
...
rule cluster_expand: ##yes
    ...
    shell:
        ...
        if [ -s "rmin.in" ]; then
        bash {script_dir}/ref_SD/sam_sd/scripts/run_rm_trf.sh rmin.in {DATA_dir}/FA/{wildcards.line}.fa  {script_dir}/ref_SD/sam_sd/scripts/trf2bed.py  ##这步为跑trf和rm 只是那些低覆盖区域的
        else
        touch rm_trf.s.bed
        fi
        ...
        cd split
        python {script_dir}/nonref_SD/scripts/gene_chr_para.py -b {base_dir}03_nonref/02_ITER/2_SD_GENE/{wildcards.line}/filt/split/ -f {DATA_dir}/FA/{wildcards.line}.fa -t 40 --script {script_dir}/nonref_SD/scripts/group1parall.r
...
rule expandt2trmtrf: ##yes
    ...
    shell:
        ...
        bash {script_dir}/nonref_SD/scripts/expandall.sh "{base_dir}01_intersim/rm_trf_cut1M.bed"  {DATA_dir}/FA/{wildcards.line}.fa.fai  {script_dir}/nonref_SD/scripts/expand_pansd.r ###bash /home/jmhan/pansdall/expandall.sh /home/jmhan/pangenome/WGSfea/allW/APG3allWnew/scripts/t2trmtrf.cut1M.bed  /home/jmhan/allfa/CHM13-APGp1-HPRCp1-HGSVCp3fa/C001-CHA-E01-Mat.fa.fai  {script_dir}/nonref_SD/scripts/expand_pansd.r
        rm rigin|| true
        rm lefin|| true
        rm *pair|| true
        rm *end.txt|| true
...
rule filter_process: ##yes ###算啦好像本来生成的新的就很少，就先这样不cluster了
    ...
    shell:
        ...
        if (( $(echo "$s1 < $s2" | bc -l) )); then ##前三列长度更短
            echo "1"
            bash {script_dir}nonref_SD/scripts/run_rm_trf.sh s1.bedo $samfa {script_dir}/ref_SD/sam_sd/scripts/trf2bed.py
            cat all1.end.statistics.filt |cut -f 1-12 |tail -n +2  >all1.end.statistics.filt.in
        else  ##后三列长度更短
            echo "2"
            bash {script_dir}nonref_SD/scripts/run_rm_trf.sh s2.bedo $samfa {script_dir}/ref_SD/sam_sd/scripts/trf2bed.py
...
rule filter_otherend:
    input:
        "{base_dir}/03_nonref/02_ITER/2_SD_GENE/{line}/processend/other.end"
    output:
        "{base_dir}/03_nonref/02_ITER/2_SD_GENE/{line}/processend/other.end.filt"
    shell:
        '''
        cd "{base_dir}/03_nonref/02_ITER/2_SD_GENE/{wildcards.line}/processend"
...
rule pair_trans: ##yes
    input:
        expand("{base_dir}/03_nonref/02_ITER/2_SD_GENE/{line}/processend/other.end.filt",base_dir=base_dir,line=lines),
        pain="{base_dir}/03_nonref/02_ITER/2_SD_GENE/allpair.sort.END.e"
    output:
        "{base_dir}/03_nonref/02_ITER/2_SD_GENE/ALLPAIR/{line}.SD.trans" 
    shell:
        '''
        mkdir -p {base_dir}/03_nonref/02_ITER/2_SD_GENE/ALLPAIR/
        cd {base_dir}/03_nonref/02_ITER/2_SD_GENE/ALLPAIR/
        cat {base_dir}/03_nonref/02_ITER/2_SD_GENE/{wildcards.line}/processend/other.end.filt  |awk '{{print $1":"$2"-"$3"\\t"$4":"$5"-"$6}}' >{wildcards.line}.SD.pair
...
rule gene_uniqtran:
    input:
        expand("{base_dir}/03_nonref/02_ITER/2_SD_GENE/ALLPAIR/{line}.SD.trans",base_dir=base_dir,line=lines),
    output:
        f"{base_dir}/03_nonref/02_ITER/2_SD_GENE/tranall.uniq"
    run:
        import subprocess
        import pandas as pd
        commo="cat "+base_dir+"/03_nonref/02_ITER/2_SD_GENE/ALLPAIR/*.trans |sort |uniq >"+base_dir+"/03_nonref/02_ITER/2_SD_GENE/tranall"
...
rule gene_align:
    input:
        ...
        corpath="{base_dir}03_nonref/02_ITER/3_PAIR_PATH/output/{sam}can_sd.corregion.bed",
        staal=f"{base_dir}/03_nonref/02_ITER/2_SD_GENE/tranall.uniq"
    output:
        "{base_dir}/03_nonref/02_ITER/2_SD_GENE/{sam}/ALIGN/{sam}.all1.end.statistics"
    shell:
        '''
        mkdir -p {base_dir}/03_nonref/02_ITER/2_SD_GENE/{wildcards.sam}/ALIGN
        ...
        cat "{base_dir}03_nonref/02_ITER/3_PAIR_PATH/output/{wildcards.sam}can_sd.corregion.bed" |tail -n +2| sed "s/_rows_/\\t/" | less -S >{wildcards.sam}.pos
        python {script_dir}nonref_SD/scripts/trans_pair_w.py --pos {wildcards.sam}.pos --W {wildcards.sam}.init.W --o {wildcards.sam}.uniqpair --trans "{base_dir}/03_nonref/02_ITER/2_SD_GENE/tranall.uniq"
...
rule run_rm_sam:
    input:
        "{base_dir}/03_nonref/02_ITER/2_SD_GENE/{sam}/ALIGN/{sam}.all1.end.statistics"
    output:
        "{base_dir}/03_nonref/02_ITER/2_SD_GENE/{sam}/ALIGN/{sam}.eebed.rev.out",
    ...
    shell:
        ...
        cd lefdic
        bash {script_dir}nonref_SD/scripts/run_rm_trf.sh lef.bed $samfa {script_dir}/ref_SD/sam_sd/scripts/trf2bed.py
        cd ..
        cd rigdic
        bash {script_dir}nonref_SD/scripts/run_rm_trf.sh rig.bed $samfa {script_dir}/ref_SD/sam_sd/scripts/trf2bed.py
        cd ..
...
rule sta_end:
    input:
        expand("{base_dir}/03_nonref/02_ITER/2_SD_GENE/{sam}/ALIGN/{sam}.eebed.rev.out",base_dir=base_dir, sam=ALLSAM)
    output:
