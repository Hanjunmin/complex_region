...
def filter_pairs(sample):
    ...
    # 执行R脚本
    cmd = ["Rscript", scr, a_txt, b_txt, output_file]  ##"/home/jmhan/pangenome/WGSfea/sdseq/filtpair.r"
...
def bash_pairs(sample):
    ...
    cmd = f"""
    cat {input_file} | tr ':' '\\t' | awk -F '\\t' 'OFS="\\t"{{gsub(/-/, "\\t", $2);gsub(/-/, "\\t", $4);gsub(/-/,"\\t",$6);print $0;}}' | awk '((!($1 == $4 && $5 > $2 && $5 < $3) && $10 >=0.1) || ($1 == $4 && $5 > $2 && $5 < $3 && $10 > 0.6)){{print $0}}' > {output_file}
    """
