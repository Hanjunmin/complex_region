...
folder_path=args[1]  ##/home/jmhan/pangenome/WGSfea/sdseq/filt/ ##/home/jmhan/pangenome/WGSfea/allW/APG3allW/scripts/process/HG00438_Pat/other/128filt0.1/filt/
data<-fread( paste(args[2],"beforegroup.txt",sep="")) ##"/home/jmhan/pangenome/WGSfea/sdseq/filt/beforegroup.txt"




###因为上面的这个如果contig很多可能会太多文件了，所以我重新设置一下 并且这里加了并行为30


library(parallel)
library(data.table)

process_value <- function(i) {
  print(i)
  x <- data[data$V1 == i,]
  if (nrow(x) == 0) return(NULL)
  dir.create(paste(folder_path, "/", i, sep = ""), showWarnings = TRUE, recursive = TRUE)
  system(paste("if [ -d ", folder_path, "/", i, " ]; then rm -f ", folder_path, "/", i, "/*; fi", sep = ""))
  fwrite(x, paste(folder_path, "/", i, "/", i, ".txt", sep = ""), sep = "\t", col.names = FALSE)
  proname <- paste(folder_path, "/", i, "/", i, ".txt", sep = "")
  pronamen <- paste(folder_path, "/", i, "/", i, ".txt", sep = "")
  command <- paste(
    "nl ", pronamen, " | ",
    "awk 'OFS=\"\\t\"{print $2,$3,$4,$5,$6,$7,$1}' | bedtools sort | ",
    "bedtools merge -i - -d 100000 -c 7 -o collapse | ",
    "less -S > ", folder_path, "/", i, "/", i, ".clu1", sep = ""
  )
  system(command)
}

mclapply(unique(data$V1), process_value, mc.cores = 30)
