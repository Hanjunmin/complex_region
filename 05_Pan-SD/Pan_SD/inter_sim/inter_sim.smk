...
# 		f"{base_dir}01_intersim/sim.out",     ##<2k
...
rule gene_interseg_homo_var:
		...
		shell:
		    ...
		    python {script_dir}/inter_sim/scripts/chm13_sim.py   --w "{base_dir}/00_preproc/WGS.window.bed" --o chm13.sim --r {DATA_dir}/FA/${{ref}}.fa  ##后面这些脚本内部的相关数据都要处理一下
...
rule gene_interseg_calsim:
		...
		shell:
			...
			python {script_dir}/inter_sim/scripts/seg1_correc_sim_addbord.py --vcf exam_clear.vcf --r {DATA_dir}/FA/${{ref}}.fa --h header --w new.win.short  --o "reg.out" --seg reg.pos ##好像有点慢
			...
			python {script_dir}/inter_sim/scripts/sim_correct_2k.py  --o reg.sdsim.2k --h header --r {DATA_dir}/FA/${{ref}}.fa
...
rule GENE_TRF_RM:
		...
		shell:
			...
			ref=$(cat {sam_list} |grep "reference" |cut -f1)
			FA={DATA_dir}/FA/${{ref}}.fa
			{RM}/RepeatMasker -x -parallel 50 -species human ${{FA}} -e ncbi
			if [ -s CHM13v2.fa.out ]; then
			    {RM}/util/RM2Bed.py CHM13v2.fa.out  ##如果没有out文件说明没有TE在这里
			else
