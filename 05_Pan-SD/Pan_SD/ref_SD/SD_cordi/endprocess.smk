...
rule refine_combine:
    ...
    shell:
        ...
        bash  {script_dir}/ref_SD/SD_cordi/scripts/endfilt.sh "{base_dir}/01_intersim/CHM13v2.fa_rm.bed" {base_dir}/01_intersim/rm_trf.bed {minimap_dir1}/res/refine {output}
...
rule merge_end:
    ...
    shell:
        """
        mkdir -p {base_dir}/02_refSD/ref_cor/filt/split/endfilt && cd {base_dir}/02_refSD/ref_cor/filt/split/endfilt
...
rule merge_end2:
    ...
    shell:
        """
        mkdir -p {base_dir}/02_refSD/ref_cor/filt/split/endfilt1 && cd {base_dir}/02_refSD/ref_cor/filt/split/endfilt1
...
rule GENE_ENDSD:
    ...
    shell:
        ...
        echo "A"
        if [ ! -s "{base_dir}/01_intersim/CHM13v2.fa_rm.bed" ]; then
             cat  T2Tchm13v1.1SD_delchrY.txt |cut -f 1-12  >end1.statistics.ascov.end
        else
            cat "{base_dir}/01_intersim/CHM13v2.fa_rm.bed" | grep Satellite | cut -f 1-3 | bedtools sort -i - |  bedtools merge -i - | bedtools coverage -header -a <(cat T2Tchm13v1.1SD_delchrY.txt ) -b -  >end1.statistics.ascov.txt
            cat  end1.statistics.ascov.txt |awk '$16<=0.7{{print $0}}' |cut -f 1-12  >end1.statistics.ascov.end  
        fi
        echo "C"
        bedtools coverage -a <(cat end1.statistics.ascov.end) -b  {base_dir}/01_intersim/rm_trf.bed|cat |awk '{{print $0,$15-$14}}' |awk '$17>=500{{print $0}}' |cut -f 1-12 >SDend1.txt
        echo "D"
        cat SDend1.txt |awk 'OFS="\\t"{{print $4,$5,$6,$1,$2,$3,$7,$8,$9,$10,$11,$12}}' |cat >SDend1.txt.lef
        echo "E"
        bedtools coverage -a <(cat SDend1.txt.lef) -b  {base_dir}/01_intersim/rm_trf.bed|cat |awk '{{print $0,$15-$14}}' |awk '$17>=500{{print $0}}'   >SDend1.txt
        cat SDend1.txt | awk '!($1==$4 && (($5<=$2 && $6>=$2)||($5<=$3 && $6>=$3)||($2<=$5 && $3>=$5)||($2<=$6 && $3>=$6)))' |sort |uniq |awk 'OFS="\\t"{{print $4,$5,$6,$1,$2,$3,$7,$8,$9,$10,$11,$12}}' >{output}
        shopt -u nullglob
        """
