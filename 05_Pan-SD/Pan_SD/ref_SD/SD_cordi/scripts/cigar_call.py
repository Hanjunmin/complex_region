...
with open(args.o, 'w') as outfile:
	outfile.write(f"ref_chr\tref_start\tref_end\tque_chr\tque_start\tque_end\tmatch_base\tmismatch_base\torient\tidentity\talign_len\n")
	i=0
	for line in all_lines:
		...
		if refs==ques and refe==quee:
			continue
		outfile.write(f"{quechr}\t{ques}\t{quee}\t{refchr}\t{refs}\t{refe}\t{m}\t{mis}\t{ori}\t{iden}\t{alignlen}\t{matchindel}\n")
