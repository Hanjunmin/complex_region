...
def process_single_folder(folder_name, base_dir,script):
    try:
...
def main():
    ...
    parser.add_argument("--threads", "-t", type=int, default=10,
                       help="并行线程数 (默认: 10)")
    parser.add_argument("--script", "-s",help="script directory")
    ...
    with ProcessPoolExecutor(max_workers=args.threads) as executor:
        # 提交所有任务
        future_to_folder = {
            executor.submit(process_single_folder, folder, args.base_dir,args.script): folder 
