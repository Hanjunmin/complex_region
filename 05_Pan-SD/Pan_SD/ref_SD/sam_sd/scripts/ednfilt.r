...
process_inte <- function(process_bef) {
  ...
  colnames(endall)[21:26]=c("rc2","rs2","re2","qc2","qs2","qe2")
  ...
  endall$label <- ifelse(
  (endall$interrl/endall$r1l == 1 & endall$interql/endall$q1l == 1), 1,
  ifelse((endall$interrl/endall$r2l == 1 & endall$interql/endall$q2l == 1), 2, NA)
)
  b2=endall[endall$label==1,c("subjectHits","queryHits")]
  colnames(b2)<-c("lar","sm")
  b1=endall[endall$label==2,c("queryHits","subjectHits")]
  colnames(b1)<-c("lar","sm")
  be<-rbind(b2,b1)
  mul=intersect(unique(be$lar), unique(be$sm))
  be=be[!(be$lar %in% mul),]
  if(length(unique(be$sm))!=0){
    del=unique(be$sm)
    process_bef=process_bef[-del,]
  }
  return(process_bef)
...
for(i in allname){
  ...
  x <- x %>% select(-combo)
  dbscan_result <- dbscan(x[,c(2,3,5,6)], eps = 1000, minPts =1)
