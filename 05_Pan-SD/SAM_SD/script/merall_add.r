...
alldf<-fread("eas_fisher.REF_refine.anno",sep="\t")
...
for(sup in unique(na.omit(samsuper$SP))){
	...
	h1 <- apply(alldf[, ..group], 1, function(x) sum(x == "HAVE"))
	h2 <- apply(alldf[, ..group], 1, function(x) sum(x == "POLY"))
	noall=no1+no2
	have=h1+h2
	haveall=have/(have+noall)
...
allx=rbind(allx,adjusted_p_values_bonferroni)
}

allxdf=data.frame(allx)
colnames(allxdf)<-c("eas_eur_p","eas_amr_p","eas_afr_p","eas_sas_p","eur_amr_p","eur_afr_p","eur_sas_p","amr_afr_p","amr_sas_p","afr_sas_p")
...
columns_to_keep <- setdiff(colnames(alldfstatisc), c(colnames(x), colnames(allxdf)))
allxdf_unique <- alldfstatisc[, ..columns_to_keep]

freall<-allxdf_unique
alldf<-cbind(freall,x)
alldf<-cbind(alldf,allxdf)


vector=c("eas_eur_p","eas_amr_p","eas_afr_p","eas_sas_p","eur_amr_p","eur_afr_p","eur_sas_p","amr_afr_p","amr_sas_p","afr_sas_p")
alldf$min_pnum=-1
alldf$max_pnum=-1
library(data.table)
setDT(alldf)
pop_mapping<- list(
  eas = grep("eas", vector, value = TRUE),
  eur = grep("eur", vector, value = TRUE),
  amr = grep("amr", vector, value = TRUE),
  afr = grep("afr", vector, value = TRUE),
  sas = grep("sas", vector, value = TRUE)
)


alldf[, c("min_pop", "max_pop") := {
  values <- .SD[, .(EAS_fre, EUR_fre, AMR_fre, AFR_fre, SAS_fre)]
  if (all(is.nan(unlist(values)))) {
    list(NA_character_, NA_character_)
  } else {
    list(
      tolower(gsub("_.*", "", names(which.min(unlist(values))))),
      tolower(gsub("_.*", "", names(which.max(unlist(values)))))
    )
  }
}, by = 1:nrow(alldf)]

# 然后批量计算 pnum
for (pop in names(pop_mapping)) {
  p_cols <- pop_mapping[[pop]]
  alldf[min_pop == pop, min_pnum := rowSums(.SD < 0.05), .SDcols = p_cols]
  alldf[max_pop == pop, max_pnum := rowSums(.SD < 0.05), .SDcols = p_cols]
}

# alldf[alldf$min_pnum>=2 |alldf$max_pnum>=2,]
...
fwrite(alldfadlab,"eas_fisher_all_n.csv",sep=",")
...
fwrite(alldf1,"eas_fisher_n.csv",sep=",")
fwrite(alldf2,"eas_fisheralsig_n.csv",sep=",")
...
# fwrite(eashigh1,"/dssg/home/acct-clsmyf/clsmyf-user1/eashigh1.csv",sep=",")
# fwrite(easlow1,"/dssg/home/acct-clsmyf/clsmyf-user1/pangenome/Pan-SD/pansam/easlow1.csv",sep=",")


# ###find
# eashigh1=fread("/dssg/home/acct-clsmyf/clsmyf-user1/eashigh1.csv")
...
# t2t=fread("/dssg/home/acct-clsmyf/clsmyf-user1/pangenome/APG_seg/T2TSD/pansd.t2t")
...
# easlow1=fread("/dssg/home/acct-clsmyf/clsmyf-user1/pangenome/Pan-SD/pansam/easlow1.csv")
# alldf=fread("allsam.csv")
