    ...
    if anno=="AJUST":
        ...
        while eid>0:
            ...
            if annoa=="INTE":
                ...
                eid=eid-1
        if ne!=0:
            return(["AJUST",start+min(sorted(positions)),ne,keya])
...
def iter(padict,chunk_size,rowdict,bech,bechname,repid,delnode,polycomplex): ###winow,全部的path，某样本大于500的path（bench）
    ...
    while start<len(bech):
        ...
        if len(na1)>2: ###感觉50个segment比较合适
            ...
            continue
        dele="false"
        tasks = [(index, row) for index, row in rowdict.items()]
        process_func1 = partial(para_sam,num1=num1,start=start,end=end)
        with mp.Pool(processes=20) as pool:
            results = pool.map(process_func1, tasks)
        annolis=[]
        for res in results:
            if res=="true":
                dele="true"
                break
            annolis.append(res)
        annolis = list(filter(None, annolis))
        if dele=="true":
            start=end
            continue
...
def splitpath(in1,keys_set,fasta_dictsplit):
...
dupelemental=[]
###把dup的区域注释一下:
def process_single_row(index_row, dupelement,keys_set,fasta_dictsplit):
    rowdict_item={}
    testlis_item=[]
    apppdata_item=[]
    dupelemental_item=[]
    index, row = index_row
    in1=str.split(row[2].replace("-",",-").replace("+",",+"),",")
    in1df,in1out=splitpath(in1,keys_set,fasta_dictsplit)
    if len(dupelement)!=0:
        positions = np.where(np.isin(in1out, list(dupelement)))[0].tolist()
        if len(positions)==0:
            rowdict_item[row[1]]=in1 
            return rowdict_item, testlis_item, apppdata_item, dupelemental_item
        arr_reshaped = np.array(positions).reshape(-1, 1)
        dbscan = DBSCAN(eps=100, min_samples=1)  ###如果位置差异大于20就分为另一类
        if len(arr_reshaped) == 0:
            print(index,"NONO")
        label=dbscan.fit_predict(arr_reshaped)
        for lab in set(label):
            ...
            apppdata_item.append(["P",str(index)+"@"+str(lab),pa,"*"])
            dupelemental_item.extend(in1[min(res):(max(res)+1)])
            in1[min(res):(max(res)+1)]=[item + "dup" for item in in1[min(res):(max(res)+1)]]
    dup_count = sum(1 for item in in1 if 'dup' in item)
    rowdict_item[row[1]]=in1 
    testlis_item.append([row[1],dup_count/len(in1)])
    print(index)
    return rowdict_item, testlis_item, apppdata_item, dupelemental_item
...
apppdatapara = []
dupelementalpara = []
tasks = [(index, row) for index, row in allpath.iterrows()]
with mp.Pool(processes=30) as pool:
    process_func = partial(process_single_row, dupelement=dupelement,keys_set=keys_set,fasta_dictsplit=fasta_dictsplit)
    results = pool.map(process_func, tasks)

# 合并结果
for rowdict_item, testlis_item, apppdata_items, dupelemental_local in results:
    rowdictpara.update(rowdict_item)
    testlispara.extend(testlis_item)
    apppdatapara.extend(apppdata_items)
    dupelementalpara.extend(dupelemental_local)

rowdict=rowdictpara
testlis=testlispara
apppdata=apppdatapara
dupelemental=set(dupelementalpara)
