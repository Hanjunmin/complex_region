...
CHR=args.chr
sdpa=pd.read_csv("ref.anchoral.rm",sep="\t",header=None)
...
def process_item(sam):
	...
	a = pybedtools.BedTool("cor_window.bed")
	result = a.intersect(b, wa=True, wb=True,f=1)
	resultdf = result.to_dataframe()
	a=resultdf[(resultdf['start']==resultdf['score']) & (resultdf['end']==resultdf['strand'])]
	b=resultdf[~((resultdf['start']==resultdf['score']) & (resultdf['end']==resultdf['strand']))]
	remove_combinations = set(a[['start', 'end']].apply(tuple, axis=1))
	b = b[~b[['start', 'end']].apply(tuple, axis=1).isin(remove_combinations)]
	resultdf=pd.concat([a,b])
