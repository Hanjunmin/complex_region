...
def motif_sim(submotif,motif_list):
    distances = []
    n = len(motif_list)
    for i in range(n):
        distance = edlib.align(motif_list[i], submotif,mode='global',task="path")['editDistance']
        distances.append(distance/max(len(motif_list[i]), len(submotif)))
    return min(distances)


def expand_motif(result_dict,motif, ref, interval,infector,alllenlist,molis):
	start,end = interval
	seq = (ref[start:(end + 1)]).upper()
	res1 = edlib.align(motif, seq, mode='HW', task="path")  # align with motif 
	subseq=seq[res1['locations'][0][0]:(res1['locations'][0][1]+1)] ##当前比对上的Motif是什么样子
	##看一下和所有的motif的编辑距离
	minmotif_sim=motif_sim(subseq,molis)
	if minmotif_sim <= infector:	
		...
		for interval in intervallist:
			expand_motif(result_dict,motif, ref, interval,infector,alllenlist,molis)
