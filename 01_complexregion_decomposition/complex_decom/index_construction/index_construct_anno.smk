...
rule gene_bub_segnum:
	...
	shell:
		...
		while read -r chr start end; do
		  ...
		  python {script_dir}/index_construction/scripts/bub/filter_path.py --i ${{segm}}.path.dou.filt --json {base_dir}01_preproc/reg.json  ##30 cores based on similarity  ${{segm}}.path.dou.filt based on the complement of paths
...
rule gene_intraseg_initpos:
	...
	shell:
		...
		python {script_dir}/index_construction/scripts/intra_seg/graph_VNTRend_onlypkl.py --script {script_dir}/index_construction/scripts/intra_seg/ --T2Tpos "{base_dir}01_preproc/reg.posCHM.txt" --w {base_dir}01_preproc/Wpath/ --json {base_dir}01_preproc/reg.json
		for i in {{0..99}};do
		echo $i
		python {script_dir}/index_construction/scripts/intra_seg/ref_extend.py --i ${{i}}pos.pkl --o ${{i}}pos.csv --script {script_dir}/index_construction/scripts/intra_seg/ --fapath {data_dir}/FA/
		done
...
rule gene_intraseg_other_CN:
		...
		shell:
			'''
			cd "{base_dir}02_index_construction/intra_seg/"
			python {script_dir}/index_construction/scripts/intra_seg/graph_VNTR_other.py --script {script_dir}/index_construction/scripts/intra_seg/ --T2Tpos "{base_dir}01_preproc/reg.posCHM.txt" --w {base_dir}01_preproc/Wpath/ --json {base_dir}01_preproc/reg.json --r {REF}
			cat *.copynumber.csv |grep  "chr" |awk '$5!=1' >INSOTHER.al
			python {script_dir}/index_construction/scripts/intra_seg/process_otherpos1.py  --script {script_dir}/index_construction/scripts/intra_seg/  --json {base_dir}01_preproc/reg.json --r {REF}
			cat samlis |tail -n +2 | parallel -j 30 \
			  "python {script_dir}/index_construction/scripts/intra_seg/process_otherpos2.py --s {{}} --o {{}}.END" --script {script_dir}/index_construction/scripts/intra_seg/  --w {base_dir}01_preproc/Wpath/ --json {base_dir}01_preproc/reg.json --fapath {data_dir}/FA/
...
rule gene_intraseg_gini:
		...
		shell:
			...
			python {script_dir}/index_construction/scripts/intra_seg/inte_intraseg.py --pos {base_dir}/01_preproc/conti_path/allsampos
