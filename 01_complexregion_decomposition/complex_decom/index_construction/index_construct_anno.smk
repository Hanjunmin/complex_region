...
rule gene_interseg_homo_var:
		input:
			{VCF}
		output:
			"{base_dir}02_index_construction/inter_seg/new.win"
		shell:
			"""
...
rule gene_interseg_calsim:
		...
		output:
			"{base_dir}02_index_construction/inter_seg/reg.sdsim.2k", ##>2k
			"{base_dir}02_index_construction/inter_seg/reg.out"       ##<2k
		shell:
			"""
		    cd "{base_dir}02_index_construction/inter_seg/"
			##<2k
...
rule gene_interseg_procesdot:
		...
		output:
		    "{base_dir}02_index_construction/inter_seg/sim.end_process.dot" 
		shell:
			"""
			cd "{base_dir}02_index_construction/inter_seg/"
...
rule gene_interseg_inte:
		...
		output:
			"{base_dir}02_index_construction/inter_seg/reg.simout" 
		shell:
			"""
			cd "{base_dir}02_index_construction/inter_seg/"
			cat > temp_script.R << 'EOF'
...
rule clear_interseg_fold: 
	...
	output:
		"{base_dir}02_index_construction/inter_seg/interseg_gene.OK"
	shell:
		"""
		cd "{base_dir}02_index_construction/inter_seg/"
...
rule gene_seq_bub:
	input:
		"{base_dir}01_preproc/cor_window.bed"
	output:
		"{base_dir}02_index_construction/bub/WGS_bubble_APG.txt"
	shell:
		"""
...
rule gene_bub_path:
		...
		output:
			"{base_dir}02_index_construction/bub/gene_path.OK"
		shell:
			"""
			cd "{base_dir}02_index_construction/bub/"
			cat > temp_script.R << 'EOF'
...
rule gene_bub_segnum:
	...
	output:
		"{base_dir}02_index_construction/bub/gene_segnum.OK"
	shell:
		"""
		cd "{base_dir}02_index_construction/bub/"
		while read -r chr start end; do
...
rule sum_bub_feature:
	...
	output:
		"{base_dir}02_index_construction/bub/bub.txt"
	shell:
		'''
		cd "{base_dir}02_index_construction/bub/"
...
rule clear_bub_fold: 
	...
	output:
		"{base_dir}02_index_construction/bub/bub_gene.OK"
	shell:
		"""
		cd "{base_dir}02_index_construction/bub/"
...
rule gene_seq_TEpos:
	...
	shell:
		...
		{RM}/RepeatMasker -x -parallel 50 -species human output_n.fasta -e ncbi
		{RM}/util/RM2Bed.py output_n.fasta.out

		bedtools getfasta -fi {REF} -bed {BED} -fo reg.fa
		{RM}/RepeatMasker -x -parallel 50 -species human reg.fa -e ncbi
		{RM}/util/RM2Bed.py reg.fa.out
...
rule gene_intraseg_initpos:
	input:
		"{base_dir}01_preproc/cor_window.bed"
	output:
		"{base_dir}02_index_construction/intra_seg/intra_seg_extend.OK"
	shell:
		'''
...
rule gene_intraseg_ref_CN:
		...
		output:
			"{base_dir}02_index_construction/intra_seg/REF.CN"
		run:
...
rule gene_intraseg_other_CN:
		...
		output:
			"{base_dir}02_index_construction/intra_seg/END.in"
		shell:
			'''
			cd "{base_dir}02_index_construction/intra_seg/"
...
rule gene_intraseg_filter_otherCN:
		...
		output:
			"{base_dir}02_index_construction/intra_seg/END.out"
		shell:
			'''
			cd "{base_dir}02_index_construction/intra_seg/"
			cat > temp_script.R << 'EOF'
...
rule gene_intraseg_gini:
		...
		output:
			"{base_dir}02_index_construction/intra_seg/ENDVNTR.AL.cov"
		shell:
			'''
			cd "{base_dir}02_index_construction/intra_seg/"
...
rule gene_intraseg_fea:
		...
		output:
			"{base_dir}02_index_construction/intra_seg/intra_seg.fea"
		run:
			import pandas as pd
...
rule clear_intra_fold: 
	...
	output:
		"{base_dir}02_index_construction/intra_seg/intra_gene.OK"
	shell:
		"""
		cd "{base_dir}02_index_construction/intra_seg/"
