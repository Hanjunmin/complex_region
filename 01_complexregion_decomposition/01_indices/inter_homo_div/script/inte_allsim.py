...
CHR=args.c
df = pd.read_csv(CHR+'.2k',sep="\t", index_col=0)
x1 = df[(df['1'] == ".") & (df['2'] != ".")]
x2=df[(df['1'] != ".") & (df['2'] == ".")]
x3=df[(df['1'] == ".") & (df['2'] == ".")]
x4=df[(df['1'] != ".") & (df['2'] != ".")]
...
# ano1=pd.read_csv('./PANSDEND/000_SD_sim/win/'+CHR+'.simn_ext',sep="\t", index_col=0)
# ano2=pd.read_csv('./PANSDEND/000_SD_sim/win/'+CHR+'.simn_long',sep="\t", index_col=0)
# Z=pd.concat([ano1,ano2])
# Z['win']=Z.index
# Z_filtered = Z[~Z['win'].isin(simdf['win'])]


ano3=pd.read_csv('./000_SD_sim/draft/'+CHR+'/'+CHR+'.simn_ext',sep="\t", index_col=0)
ano3['win']=ano3.index
