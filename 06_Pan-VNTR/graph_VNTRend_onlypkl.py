...
def process_other(target_value):
	...
	for nanclus in set(nanx['cluster']):
		...
		if index1!=0:
			if index1-1!=0:
				...
				if reflef_mark==0:
					...
					if any(cenlines <= int(point) <= cenlinee for point in reflef_seg['T2Tpos']):
						lefseq="N"
			else:
				lefseq="N"
		else:
		...
		if index2!=(len(df2hap)-1):
			...
			if refrig_mark==0:
				...
				if any(cenlines <= int(point) <= cenlinee for point in refrig_seg['T2Tpos']):
					rightseq="N"
		else:
			rightseq="N"
		...
		if len(insfa)<5000:
				...
				if not (int(trflis[1]) <= splice[0] or int(trflis[0]) >= splice[1] or (int(trflis[0]) >= splice[0] and int(trflis[1]) <= splice[1])):
					...
					if res1['editDistance'] <= 0.3*len(motif) and (sorted(res1['locations'])[-1][1]-sorted(res1['locations'])[0][0])/len(motif)>=0.8 and (sorted(res1['locations'])[-1][1]-sorted(res1['locations'])[0][0])/len(motif)<2:
						...
						if xxx==None:
							...
							addlist=[CHRn,xxx[0],xxx[1],trflis[2],motif,cn,target_value,lefseqnew+"*******"+insfa+"*******"+rightseqnew,insfa,nanclus,splice,keyNod]
							othervntr.append(addlist)
				if (int(trflis[0]) >= splice[0] and int(trflis[1]) <= splice[1]):
					motif=trflis[13]
					cn=trflis[3]
					if lefseq!="N":
						addlist=[CHRn,list(reflef_seg['end'])[-1],"INS",trflis[2],motif,cn,target_value,lefseqnew+"*******"+insfa+"*******"+rightseqnew,insfa,nanclus,splice,keyNod]
						othervntr.append(addlist)
					if rightseq!="N" and lefseq=="N":
						addlist=[CHRn,list(refrig_seg['T2Tpos'])[0],"INS",trflis[2],motif,cn,target_value,lefseqnew+"*******"+insfa+"*******"+rightseqnew,insfa,nanclus,splice,keyNod]
						othervntr.append(addlist)
		else:
			input_file=CHRn+target_value+".fa"
			...
			for ran in range(0,len(result1)):
				...
				if ((int(trflis[0]) <= splice1[0] and int(trflis[1]) >= splice1[0])):
					...
					fa = pysam.FastaFile(fafile)
					name =CHRn + ":" + str(int(t2tpos[0])) + "-" + str(int(t2tpos[1]))
					addlist=[CHRn,t2tpos[0],t2tpos[1],trflis[2],motif,cn,target_value,lefseqnew+"*******"+insfanew1+"*******",insfanew1,nanclus,splice1,keyNod]
					othervntr.append(addlist)
				if ((int(trflis[0]) >= splice1[0])):
					...
					fa = pysam.FastaFile(fafile)
					name =CHRn + ":" + str(int(t2tpos[0])) + "-" + str(int(t2tpos[1]))
					addlist=[CHRn,t2tpos[0],"INS",trflis[2],motif,cn,target_value,lefseqnew+"*******"+insfanew1+"*******",insfanew1,nanclus,splice1,keyNod]
					othervntr.append(addlist)
			for ran in range(0,len(result2)):
				...
				if ((int(trflis[0]) <= splice2[0] and int(trflis[1]) >= splice2[0])):
					...
					addlist=[CHRn,t2tpos[0],t2tpos[1],trflis[2],motif,cn,target_value,"*******"+insfanew2+"*******"+rightseqnew,insfanew2,nanclus,splice2,keyNod1]
					othervntr.append(addlist)
				if ((int(trflis[1]) <= splice2[0])):
					...
					fa = pysam.FastaFile(fafile)
					name =CHRn + ":" + str(t2tpos[0]) + "-" + str(t2tpos[1])
					addlist=[CHRn,t2tpos[0],"INS",trflis[2],motif,cn,target_value,"*******"+insfanew2+"*******"+rightseqnew,insfanew2,nanclus,splice2,keyNod1]
					othervntr.append(addlist)
	return othervntr
	# 	else:
	# 		input_file=CHRn+target_value+".fa"
	...
	# 		insfanew2=insfa[-5000:]
	# 		rightseqnew=rightseq[:5000]
	# 		allseq1=lefseqnew+insfanew1+rightseqnew
	# 		splice1=[len(lefseqnew),len(lefseqnew+insfanew1)]
	# 		allseq2=lefseqnew+insfanew2+rightseqnew
	# 		refseq=lefseqnew+rightseqnew
	# 		splice2=[len(lefseqnew),len(lefseqnew+insfanew2)]
	# 		if lefseq!="N":
	# 			result_rows=[]
	# 			cumulative_sum=0 
	# 			for idx, row in reflef_seg[::-1].iterrows():  # 倒序遍历
	...
	# 		for ran in range(0,len(result1)):
	# 			trflis=result1[ran].split(' ')
	# 			if not (int(trflis[1]) <= splice1[0] or int(trflis[0]) >= splice1[1] or (int(trflis[0]) >= splice1[0] and int(trflis[1]) <= splice1[1])):
	# 				motif=trflis[13]
	# 				cn=trflis[3]
	# 				if lefseq!="N" and rightseq!="N":
	# 					a=(refrig_segn['T2Tpos'].dropna().tolist() + reflef_segn['T2Tpos'].dropna().tolist())
	...
	# 						continue
	# 					else:
	# 						addlist=[CHRn,xxx[0],xxx[1],trflis[2],motif,cn,target_value,lefseqnew+"*******"+insfanew1+"*******"+rightseqnew,insfanew1,nanclus,splice1,keyNod]
	# 						othervntr.append(addlist)
	# 			if (int(trflis[0]) >= splice1[0] and int(trflis[1]) <= splice1[1]):
	# 				motif=trflis[13]
	# 				cn=trflis[3]
	# 				if lefseq!="N":
	# 					addlist=[CHRn,list(reflef_seg['end'])[-1],"INS",trflis[2],motif,cn,target_value,lefseqnew+"*******"+insfa+"*******"+rightseqnew,insfa,nanclus,splice1,keyNod]
	# 					othervntr.append(addlist)
	# 				if rightseq!="N" and lefseq=="N":
	# 					addlist=[CHRn,list(refrig_seg['T2Tpos'])[0],"INS",trflis[2],motif,cn,target_value,lefseqnew+"*******"+insfa+"*******"+rightseqnew,insfa,nanclus,splice1,keyNod]
	# 					othervntr.append(addlist)
	# 		for ran in range(0,len(result2)):
	# 			trflis=result2[ran].split(' ')
	# 			if not (int(trflis[1]) <= splice2[0] or int(trflis[0]) >= splice2[1] or (int(trflis[0]) >= splice2[0] and int(trflis[1]) <= splice2[1])):
	# 				motif=trflis[13]
	# 				# if len(motif)<6:
	# 				# 	continue
	# 				cn=trflis[3]
	...
	# 						continue
	# 					else:
	# 						addlist=[CHRn,xxx[0],xxx[1],trflis[2],motif,cn,target_value,lefseqnew+"*******"+insfanew2+"*******"+rightseqnew,insfanew2,nanclus,splice2,keyNod]
	# 						othervntr.append(addlist)
	# 			if (int(trflis[0]) >= splice2[0] and int(trflis[1]) <= splice2[1]):
	# 				motif=trflis[13]
	# 				cn=trflis[3]
	# 				if lefseq!="N":
	# 					addlist=[CHRn,list(reflef_seg['end'])[-1],"INS",trflis[2],motif,cn,target_value,lefseqnew+"*******"+insfa+"*******"+rightseqnew,insfa,nanclus,splice2,keyNod]
	# 					othervntr.append(addlist)
	# 				if rightseq!="N" and lefseq=="N":
	# 					addlist=[CHRn,list(refrig_seg['T2Tpos'])[0],"INS",trflis[2],motif,cn,target_value,lefseqnew+"*******"+insfa+"*******"+rightseqnew,insfa,nanclus,splice2,keyNod]	
	# 					othervntr.append(addlist)
	# return othervntr
...
for sams in range(len(split_lists)): # for samlis in split_lists:
	...
	with open(CHRn+"_"+str(samlisid)+"alvntrsampos_n.pkl", "wb") as f:
