...
def generate_motif_pos(iterid,trans):
    ...
    if en>maxlength:
        en=maxlength
    T2TSeq = fa.fetch(region=row[2]+":"+str(st)+"-"+str(en))
    ref=T2TSeq.upper()
    if trans==2: ##chan2
...
# initcn="./project/ENDVNTR/STRVNTR130/chr1.STR_VNTR.out" ##获取对应的motif
initcn=args.init
file_name=args.i
T2Tpos = pd.read_csv(initcn,sep="\t",header=0)
...
if len(results)!=0:
    ...
    xxdf['ns'] = pd.to_numeric(xxdf[3]) + xxdf[5]
    xxdf['ne'] = pd.to_numeric(xxdf[3]) + xxdf[6]
    xxdf['ratio']=abs(xxdf['inite']-xxdf['inits'])/abs(xxdf['ne']-xxdf['ns'])
    delregion=xxdf[xxdf['ratio']<0.001]
    ...
    for sam in set(xxdf[1]):
        ...
        for index,row in xxdfsam.iterrows():
            # print(index)
            maxlength=fa.get_reference_length(row[2])
            motiflen=len(motifdic[row[0]][0])
            ...
            ####确定初始motif和对应的拷贝数
            T2TSeq = fa.fetch(region=row[2]+":"+str(stal)+"-"+str(enal))
            T2TSeq=T2TSeq.upper()
            cnmax=0
            cnmotif="a"
            for motif in motifdic[row[0]]:
                ...
                if alllenlist2!=[]:
                    if cn2>cnmax:
                        ...
                        cnmotif=motif
            if cnmax==0:
                poslis=[row[0],0,sam,row[2],row['ne'],row['ns'],motif,-1]
            else:
                # initcn=cnmax  
                # outmax="a"
                # nextcn=initcn+1
                ...
                if outmax[0][2]-outmax[0][1]!=0:
                    ...
                    simindex=-1
                if trans==2:
                    poslis=[row[0],outmax[0][0],sam,row[2],st+outmax[-1]-outmax[0][2],st+outmax[-1]-outmax[0][1],",".join(molis),simindex]
                else:
                    poslis=[row[0],outmax[0][0],sam,row[2],st+outmax[0][1],st+outmax[0][2],",".join(molis),simindex]
            alldflis.append(poslis)
